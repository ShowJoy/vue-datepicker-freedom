...
export default {
  ...
  methods: {
    ...
    setDate (timestamp) {
      ...
      // save checked date
      let index = this.checkedDate.indexOf(timestamp);
      if (index === -1) {
        this.checkedDate.push(timestamp);
      } else {
        this.checkedDate.splice(index, 1);
      }
      this.setPageDate(date)
      this.$emit('selected', new Date(date), this.checkedDate)
      this.$emit('input', new Date(date), this.checkedDate)
    },
    clearDate () {
